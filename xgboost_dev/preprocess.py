...
def load_paysim_data(file_path):
    ...
    dtypes = {
        ...
        'type': 'category',
        'amount': np.float32,
        'oldbalanceOrg': np.float32,    #!컬럼명 확인 필요
        'newbalanceOrig': np.float32,   #!컬럼명 확인 필요
...
def engineer_features(df):
    ...
    # 'type' 컬럼을 숫자로 변환 (TRANSFER: 0, CASH_OUT: 1 등)
    df_filtered['type'] = df_filtered['type'].cat.codes
    
    return df_filtered


# *학습 전 isFraud 라벨의 비율을 확인하여 scale_pos_weight 값을 결정*
